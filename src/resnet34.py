...
class Resnet34:
    ...
    def build(self):
        ...
        current_tensor = self._residual_block(current_tensor, "res_block1", [3, 3, 64, 64], cnt = 3)
        current_tensor = self._residual_block(current_tensor, "res_block3", [3, 3, 64, 128], div = True, cnt = 4)
        current_tensor = self._residual_block(current_tensor, "res_block5", [3, 3, 128, 256], div = True, cnt = 6)
        current_tensor = self._residual_block(current_tensor, "res_block7", [3, 3, 256, 512], div = True, cnt = 3)
    ...
    def _residual_block(self, input_tensor, name, kernel, div = False, cnt = 2):
        if div:
        ...
        for i in range(2, cnt):
            current_tensor = self.Utils.conv2d_bn_relu(name + "_conv" + str(i), current_tensor, kernel, [1, 1])
        current_tensor = self.Utils.conv2d_bn(name + "conv" + str(cnt), current_tensor, kernel, [1, 1])
        current_tensor = tf.add(current_tensor, shortcut)
        return tf.nn.relu(current_tensor)
   

if __name__ == "__main__":
    resnet34 = Resnet34([None, 32, 32, 3], [None, 100])
    resnet34.build()
