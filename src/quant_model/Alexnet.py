...
class Alexnet:
    ...
    def build(self):
        ...
        current_tensor = self.Utils.quant_conv2d_bn_relu("conv5", current_tensor, [3, 3, 128, 128], [1, 1], quant_bits = self.quant_bits)
        ...
        current_tensor = self.Utils.quant_fc_relu("fc1", current_tensor, [512, 1024], quant_bits = self.quant_bits)
        current_tensor = self.Utils.quant_fc_relu("fc2", current_tensor, [1024, 1024], quant_bits = self.quant_bits)
        self.result    = self.Utils.quant_fc(current_tensor, [1024, 100], quant_bits = self.quant_bits)
