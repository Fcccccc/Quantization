...
class Alexnet:
    ...
    def build(self):
        ...
        current_tensor = self.Utils.quant_fc_relu("fc1", current_tensor, [512, 1024], quant_bits = self.quant_bits, enable_prune = True)
        current_tensor = self.Utils.quant_fc_relu("fc2", current_tensor, [1024, 1024], quant_bits = self.quant_bits, enable_prune = True)
        self.result    = self.Utils.quant_fc(current_tensor, [1024, 100], quant_bits = self.quant_bits, enable_prune = True)
