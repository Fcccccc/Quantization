...
# tf.train.MomentumOptimizer top1_acc 68.15 top5_acc 88.87
# tf.train.AdamOptimizer top1_acc 71.13 top5_acc 91.27
# three fc is better?
...
class Vgg:
    ...
    def conv2d(self, input_tensor, input_tensor_channel, output_tensor_channel, kernel_size = 3):
    ...
    def conv_bn_relu(self, name, input_tensor, input_tensor_channel, output_tensor_channel, kernel_size = 3):
        with tf.variable_scope(name, reuse = tf.AUTO_REUSE): 
            conv = self.conv2d(input_tensor, input_tensor_channel, output_tensor_channel, kernel_size = kernel_size)
            conv = tf.contrib.layers.batch_norm(conv, scale = True, is_training = True, updates_collections= None)
            return tf.nn.relu(conv)
            
        
    def conv_group(self, input_tensor, input_channel, output_channel, num, name, pooling_type = "MAX", final_kernel_size = 3):
        conv_cnt = 0
        with tf.variable_scope(name, reuse = tf.AUTO_REUSE):
            conv = self.conv_bn_relu(str(conv_cnt), input_tensor, input_channel, output_channel)
            conv_cnt += 1
            for i in range(1, num):
                if i == num - 1:
                    conv = self.conv_bn_relu(str(conv_cnt), conv, output_channel, output_channel, final_kernel_size)
                else:
                    conv = self.conv_bn_relu(str(conv_cnt), conv, output_channel, output_channel)
                conv_cnt += 1
            if pooling_type == "MAX":
    ...
    def fc_bn_relu(self, input_tensor, input_channel, output_channel, name):
        with tf.variable_scope(name):
            current = self.fc(input_tensor, input_channel, output_channel, name = "fc")
    ...
    def fc(self, input_tensor, input_channel, output_channel, name):
        with tf.variable_scope(name):
    ...
    # net
        ...
        #conv2
        current = self.conv_group(name = "conv2", input_tensor = current, input_channel = 64, output_channel = 128, num = 2)
        #conv3
        current = self.conv_group(name = "conv3", input_tensor = current, input_channel = 128, output_channel = 256, num = 3, final_kernel_size = 1)
        #conv 4
        current = self.conv_group(name = "conv4", input_tensor = current, input_channel = 256, output_channel = 512, num = 3, final_kernel_size = 1)
        #conv 5
        current = self.conv_group(name = "conv5", input_tensor = current, input_channel = 512, output_channel = 512, num = 3, pooling_type = "AVG", final_kernel_size = 1)
        #reshape for fc
        current = tf.reshape(current, [-1, 512])
        #fc_bn_relu1
        current = self.fc_bn_relu(name = "fc_bn_relu1", input_tensor = current, input_channel = 512, output_channel = 4096)
        #fc_bn_relu2
        current = self.fc_bn_relu(name = "fc_bn_relu2", input_tensor = current, input_channel = 4096, output_channel = 4096)
        #fc1
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(1, 125 + 1):
                ...
                for x, y in gen():
                    cnt += 1
                    index += 1
                    s, _, train_loss, train_acc, train_top5_acc = sess.run([merged_summary, self.train_step, self.loss, self.top1_acc, self.top5_acc], feed_dict = {self.X: x,self.Y: y, self.lr: lr})
                    if cnt % 5 == 0:
                ...
                if epoch % 5 == 0:
                    ...
                    test_loss, test_acc, test_top5_acc = sess.run([self.loss, self.top1_acc, self.top5_acc], feed_dict = {self.X: x, self.Y: y})
